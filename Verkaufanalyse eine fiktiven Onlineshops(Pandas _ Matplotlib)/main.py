...
def Aufgabe7 ():
    ...
    try:
        df = pd.read_csv(Verkauf, encoding='utf-8-sig', on_bad_lines='skip', sep=None, engine='python')
        MehrAlsUmsatzFünfhundert = df[df["Umsatz"] > 500]
        AlleVerkäufeAusMärz = df[
            (df["Datum"] >= '2024-03-01') &
            (df["Datum"] <= '2024-03-31')
            ]
        NichtMehrAlsEinMalGekauft = df[df["Menge"] < 1]

        print(f"Einkäufe mit mehr als 500€ Umsatz:\n {MehrAlsUmsatzFünfhundert}")
        print(f"Alle Verkäufe aus dme März:\n {AlleVerkäufeAusMärz}")
        print(f"Nicht mehr als 1 mal gekauft:\n {NichtMehrAlsEinMalGekauft}")
...
def Hauptmenu():
    ...
    print("7. Aufgabe 7: Filteranalysen")
    try:
        auswahl = int(input("Bitte wählen Sie eine Aufgabe (1-7): "))
        if auswahl in aufgaben:
            aufgaben[auswahl]()
        else:
            print("Ungültige Auswahl. Bitte wählen Sie eine Zahl zwischen 1 und 7.")
    except ValueError:
